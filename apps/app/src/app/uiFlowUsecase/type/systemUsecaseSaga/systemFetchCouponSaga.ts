...
export function* systemFetchCouponSaga() {
  ...
  const oldNotifications: GetNotificationResponse = yield select(
    (state: RootState) => state.indexPage.notification
  );
  const lastestNotificationRespons: GetNotificationResponse = yield call(
    Service.IndexService.getNotification,
    null
  );
  ...
  if (notifications.length > 0) {
    ...
    yield put(
      indexPageSlice.actions.updateNotification(removeFirstNotifications)
    );
  }
}
