...
export const getAppInfo = (): IAndroidAppInfo => {
  ...
  if (!window['AppInfoTask'] || !window['AppInfoTask']['getAppInfo']) {
    if (environment.country === 'in') {
      ...
      if (AppEnvironment.isLocalhost()) {
        appInfo =
          NULL_DEFAULT_APP_INFO_TASK[AllCountriesEnum.pakistan].localhost;
      } else {
        appInfo = AppEnvironment.isDev()
          ? NULL_DEFAULT_APP_INFO_TASK[AllCountriesEnum.pakistan].dev
          : NULL_DEFAULT_APP_INFO_TASK[AllCountriesEnum.pakistan].prod;
      }
      ...
      if (AppEnvironment.isLocalhost()) {
        ...
        appInfo = AppEnvironment.isDev()
          ? NULL_DEFAULT_APP_INFO_TASK[AllCountriesEnum.mexico].dev
          : NULL_DEFAULT_APP_INFO_TASK[AllCountriesEnum.mexico].prod;
      }
    } else if (environment.country === PhilippinesCountry.country) {
      if (AppEnvironment.isLocalhost()) {
        appInfo =
          NULL_DEFAULT_APP_INFO_TASK[AllCountriesEnum.philippines].localhost;
      } else {
        appInfo = AppEnvironment.isDev()
          ? NULL_DEFAULT_APP_INFO_TASK[AllCountriesEnum.philippines].dev
          : NULL_DEFAULT_APP_INFO_TASK[AllCountriesEnum.philippines].prod;
      }
    } else {
