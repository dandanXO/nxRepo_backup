...
const Coupon = (props: ICouponProps) => {
    ...
    return (
        <div className={cx(`flex m-2 grow  `, { 'opacity-50': status === 'unUsable'})}>
            <div className={cx(`flex flex-col p-2 text-left  border  border-r-0 grow rounded-l-lg relative `,
                    [typeStyle.lightContent]
                )}
            >
                {layoutType === 1 &&
                    <div className={cx('absolute w-[20px] h-[10px] rounded-b-full  border border-solid top-[-1px]  right-[-11px] bg-white border-t-0 ', {
                        'border-primary-main': status !== 'disabled',
                        'border-cstate-disable-main': status === 'disabled',
                    })}>
                    </div>}
                {layoutType === 1 && <div className={cx('absolute w-[20px] h-[10px] rounded-t-full border border-solid border-primary-main bottom-[-1px] right-[-11px] bg-white border-b-0', {
                    'border-primary-main': status !== 'disabled',
                    'border-cstate-disable-main': status === 'disabled',
                })}></div>}

                <div className={cx(`font-bold`, 'text-xs', {
            ...
            {layoutType === 1 && <div className={cx('w-[1px] border-l-[1px] border-0 border-dashed  border-solid border overflow-hidden', {
                'border-primary-main': status !== 'disabled',
                'border-cstate-disable-main': status === 'disabled',
            })}></div>}
            <div className={cx(
