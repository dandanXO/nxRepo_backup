...
const SystemCouponModal = () => {
  ...
  const handleOnClick = () => {
    ...
    if (isForwardBrowser) {
      ...
      const nextUrl = {
        FORWARD_COUPON_PAGE: `${
          PageOrModalPathEnum.MyCouponListPage
        }?token=${getToken()}`,
        ...
        FORWARD_BIND_BANKCARD: `${
          PageOrModalPathEnum.BindBankcard
        }?token=${getToken()}`,
  ...
  return (
    <div
      className={
        'system-coupon-modal fixed top-0 bottom-0 z-10 flex h-full w-screen flex-col items-center justify-center bg-black bg-opacity-80 p-5'
      }
    >
      <div
        className={
          'modal-inner w-11/12 rounded-lg bg-white px-8 py-6 text-center'
        }
      >
        ...
        <div className={'my-5 flex flex-col text-sm'}>
          {systemCouponModal.webUrl ? (
            isForwardBrowser ? (
              ...
              <CouponModalContentAndroidWebviewPage />
            )
          ) : systemCouponModal.action === 'FORWARD_BIND_BANKCARD' ? (
            <div
              className="text-left"
              dangerouslySetInnerHTML={{
                __html: systemCouponModal.text.replace(/\n/g, '<br/>'),
              }}
            />
          ) : (
            <div>{systemCouponModal.text}</div>
