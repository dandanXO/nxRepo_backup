...
export const PakistanBindBankAccountPage = (props: IUseBindBankAccountPage) => {
  ...
  useEffect(() => {
    ...
    if (walletDropListData.length !== 0) {
      const walletList = walletDropListData.map((i) => ({
        value: i.code,
        label: i.displayName,
      }));
  ...
  return (
    <>
      ...
      <div
        className={
          'bg-secondary-assistant text-secondary-main mb-2 rounded-md px-3 py-2 text-xs'
        }
      >
        <span>
          If you wish to borrow an amount greater than 20,000 Rupees, please
          select “Bank Card” as your preferred payment method.
        </span>
      </div>
      <div
        className={
          'bg-cstate-info-variant text-cstate-info-main mb-4 rounded-md px-3 py-2 text-xs'
        }
      >
