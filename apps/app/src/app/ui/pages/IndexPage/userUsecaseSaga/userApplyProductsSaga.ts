...
import {
  IndexPageSagaAction,
  UserApplyProductActionPayload,
} from './indexPageActions';
...
function* callAndroidFunctionToUploadUserPhoneData() {
  ...
  if (
    ...
    // NOTICE: 新增邏輯錯誤
    const message =
      'Call Android IndexTask.uploadKycBackgroundData unsuccessfully';
    console.log(message);
  ...
  try {
    yield put(
      routerActions.push(
        ...
        }/quick-repayment-modal?token=${getToken()}`
      )
    );
    ...
    const isSystemEnableToForceApplyForOldUser: boolean = yield select(
      (state: RootState) => state.indexPage.indexAPI?.oldUserForceApply
    );
    ...
    if (!isSystemEnableToForceApplyForOldUser) {
      ...
      const bankCardListResponse: GetBankCardListResponse = yield call(
    ...
    if (ignoreToLetUserToAuthenticate) {
      ...
      wasUserUploadedDataOrSystemIgnoreUserToUpload = yield call(
        callAndroidFunctionToUploadUserPhoneData
      );
    }
    ...
    if (wasUserUploadedDataOrSystemIgnoreUserToUpload) {
      ...
      const selectedBankcardID: number = yield select(
        (state: RootState) =>
          state.model.simpleQuickRepaymentModal.selectedBankcardId
      );
