...
// NOTE: 無使用
export const getQuotaModelStatusActions =
  createAsyncThunk<GetQuotaModelStatusRequest>(
    '/api/v3/loan/quota-model-status',
    async (thunkAPI) => {
      const response: GetQuotaModelStatusResponse | null =
        await Service.LoanService.getQuotaModelStatus({});
      return response;
    }
  );
