...
export const getTemplate1AdTemplate1Data = (
    ...
    return {
        ...
        cards: ads
            ...
            .map(
                (
                    item: ActivityBanner<AdTemplate1BrandCard, AdTemplate1Card>
                ) => {
                    return {
                        action: item.action,
                        actionUrl: item.actionUrl,
                        actionName: (item?.payload as AdTemplate1Card)
                            ?.actionName,
                        title: item?.payload?.title,
                        description1: (item?.payload as AdTemplate1Card)
                            ?.description1,
                        description2: (item?.payload as AdTemplate1Card)
                            ?.description2,
                    };
                }
            ),
