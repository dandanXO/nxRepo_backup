...
import {
    ...
    RecommendProduct,
} from "../../../api/GetPersonalLoanRecommend";
...
const ProductAdModalListPage = () => {
    ...
    const data = useSelector(
        (state: RootState) => state.personalLoanRecommendSlice.data
    );
    const [personalLoanInfo, setCurrentData] =
        useState<GetPersonalLoanRecommendResponse>();
    // console.log("personalLoanInfo", personalLoanInfo)

    const pageStatus: STATE = useSelector(
        (state: RootState) => state.personalLoanRecommendSlice.status
    );
    ...
    const countDown = (countDownStr: string) => {
        ...
        intervalIDRef.current = setInterval(() => {
            const timeInfo =
                getTimeInfoBetweenCurrentAndCountDown(countDownStr);
            setTimeString(timeInfo.time);
            if (timeInfo.end) {
                cancelCountDown();
                dispatch(
                    PersonalRecommendActions[STATE.OVERDUE](STATE.OVERDUE)
                );
            }
        }, 1000);
    ...
    useEffect(() => {
        if (pageStatus === "COUNTDOWN") {
            if (personalLoanInfo?.quotaExpireTime) {
                // NOTICE: real world
                const expiredTime = personalLoanInfo?.quotaExpireTime
                    ? personalLoanInfo?.quotaExpireTime.split(".")[0]
                    : "";
                const countDownStr = moment(expiredTime).format(
                    "YYYY-MM-DD HH:mm:ss"
                );
            ...
            personalLoanInfo?.products?.map((product) => {
                if (!end) {
                    if (
                        ...
                        currentTotalPrice + product?.loanableAmount <=
                            currentValue
                    ) {
                        currentTotalPrice =
                            currentTotalPrice + product?.loanableAmount;
    ...
    const onUserClickToApply = useCallback(() => {
        if (pageStatus === STATE.COUNTDOWN) {
            ...
            cancelCountDown();
            dispatch(
                PersonalRecommendActions[STATE.APPLY_OVERDUE](
                    STATE.APPLY_OVERDUE
                )
            );
        }
                ...
                <StyledSlider>
                    <div className="info">
                        ...
                        <div className="price">
                            ...
                            {personalLoanInfo?.quotaBar?.max === 0
                                ? "-"
                                : currentValue}
                        </div>
                    </div>
                        ...
                        {pageStatus === STATE.OVERDUE && (
                            <div className="button-container">
                                ...
                                <Button
                                    color="#fff"
                                    background="#F82626"
                                    onClick={onUserClickToLoadRecommendation}
                                >
                    ...
                    <StyledList>
                        {pageStatus === STATE.COUNTDOWN &&
                            productList?.length === 0 && (
                                <div className="container">
                                    <p>
                                        Insufficient funds to provide product
                                        recommendations. Please adjust your
                                        budget accordingly.
                                    </p>
                                </div>
                            )}
                        ...
                        {pageStatus === STATE.OVERDUE && maxQuota > 0 && (
                            <div className="container">
                                <div className="overdue">
                                    <div className={"p1"}>
                                        Your current discount limit has been
                                        exhausted We are reviewing the amount
                                        you can borrow for you, please wait
                                        patiently for 30 seconds to two minutes.
                                    </div>
                                    <div className={"p2"}>
                                        To avoid errors, we recommended that you
                                        stay on this screen.
                                    </div>
                                </div>
                            </div>
                        ...
                        {pageStatus === STATE.OVERDUE_LOADING && (
                            <div className="container">
                                <div className="overdue">
                                    <div className={"p1"}>
                                        Please wait patiently for 30 seconds to
                                        2 minutes while we review the loan
                                        amount you are eligible for as quickly
                                        as possible.
                                    </div>
                                    <div className={"p2"}>
                                        To prevent errors, please remain on this
                                        screen.
                                    </div>
                                </div>
                            </div>
                ...
                <ApplyContainer>
                    ...
                    {pageStatus === STATE.APPLY && (
                        <div className="content">
                            <p className="p1">
                                After the review is successful, you can view
                                your loan order in the loan record.
                            </p>
                        </div>
                    )}
                </ApplyContainer>
            ...
            <EmbedPage>
                <ApplyContainer>
                    ...
                    {pageStatus === STATE.APPLY_OVERDUE && (
                        <div className="content">
                            <p className="p1">
                                The limited-time promotional loan scheme has
                                expired, and the loan amount will be based on
                                the latest review results.
                            </p>
                            <p className="p2">
                                Please be patient while waiting for the review
                                results. After the review is successful, you can
                                view your loan order in the loan record.
                            </p>
                        </div>
                    )}
                </ApplyContainer>
            ...
            <EmbedPage>
                <ApplyContainer>
                    ...
                    {pageStatus === STATE.APPLY_REPEAT && (
                        <div>
                            <p className="p1">
                                Please do not resubmit and wait patiently.
                            </p>
                        </div>
                    )}
                </ApplyContainer>
...
const getTimeInfoBetweenCurrentAndCountDown = (quotaExpireTime: string) => {
    ...
    const time = `${padStartZero(hours)} : ${padStartZero(
        minutes
