...
class ReactSlider extends React.Component {
    ...
    static getDerivedStateFromProps(props, state) {
        // console.log("[ReactSlider] getDerivedStateFromProps.props.max",props.max);
    ...
    resize() {
        ...
        const thumbSize =
            thumbRect[sizeKey.replace("client", "").toLowerCase()];
    ...
    fireChangeEvent(event) {
        if (this.props[event]) {
            this.props[event](
                prepareOutValue(this.state.value),
                this.state.index
            );
        }
    }
