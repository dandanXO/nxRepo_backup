...
const RepaymentAdsModal = (props: RepaymentAdsModalProps) => {
    const { startRequest, endRequest, isRequestPending } = useLockRequest(
    ...
    const handleConfirm = () => {
        const formBalanceValue = Number(
            props.balance.replace(`${environment.currency}`, "")
        );
        console.log("[repay] formBalanceValue", formBalanceValue);
        if (formBalanceValue === 0) return;
        props.handlePostRepayCreate(false, false, formBalanceValue);
    };
                            ...
                            <SectionButton>
                                <Brand>
                                    <CloseICON
                                        src={CloseICONPng}
                                        onClick={() => {
                                            props.setShowRepaymentAdsModal(
                                                false
                                            );
