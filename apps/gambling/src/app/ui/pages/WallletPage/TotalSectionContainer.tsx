...
export const TotalSectionContainer = () => {
  ...
  return (
    <StyledTotalSectionContainer className={"flex flex-col text-white mb-6"}>
      ...
      <TotalSectionTopContent className={"flex-1 flex flex-col p-4 md:py-0 md:flex-row  justify-around items-center px-5 font-bold w-full"}  onClick={() => {
        if(isMobile) setExpand(!expand);
      }}>
        ...
        <div className={"w-full flex-[2] flex flex-row "}>
          <div className={"flex-1 flex flex-col justify-center items-center"}>
            <div className={"flex flex-col text-xl md:text-3xl"}>{totalBalanceSheetValue.toFixed(2)}</div>
            <div className={"flex flex-col text-sm md:text-2xl"}>Balanço Total</div>
          </div>

          <div className={"flex-1 flex flex-col justify-center items-center"}>
            <div className={"flex flex-col text-xl md:text-3xl"}>{totalReasableValue.toFixed(2)}</div>
        ...
        <TotalSectionBottomContent className={"flex flex-row flex-wrap justify-between items-center py-5 px-5 text-base md:text-medium"}>
          ...
          <div className={"w-full flex flex-row flex-nowrap flex-1 basis-1/2 justify-evenly mr-1"}>
            ...
            <div className={"basis-1/3 max-w-[33%] shrink-0 flex flex-col justify-center items-center "}>
              <div className={"flex flex-col text-xl md:text-2xl"}>{totalBalanceSheetValue.toFixed(2)}</div>
              <div className={"flex flex-col text-sm md:text-base"}>Balanço</div>
            </div>

            <div className={"basis-1/3 max-w-[33%] shrink-0 flex flex-col justify-center items-center "}>
              <div className={"flex flex-col text-xl md:text-2xl"}>{totalReasableValue.toFixed(2)}</div>
          ...
          <div className={"w-full flex flex-row flex-nowrap flex-1 basis-1/2 justify-evenly mr-1"}>
            ...
            <div className={"basis-1/3 max-w-[33%] shrink-0 flex flex-col justify-center items-center "}>
              <div className={"flex flex-col text-xl md:text-2xl"}>{accountPromotedSwingValue.toFixed(2)}</div>
              <div className={"flex flex-col text-sm md:text-base"}>Balanço</div>
            </div>

            <div className={"basis-1/3 max-w-[33%] shrink-0 flex flex-col justify-center items-center"}>
              <div className={"flex flex-col text-xl md:text-2xl"}>{accountPromotedWithdrawableValue.toFixed(2)}</div>
              <div className={"flex flex-col text-sm md:text-base"}>Retirável</div>
            </div>
