...
export const WithdrawPanel = (props: IWithdrawPanelCommon) => {
  ...
  const { withdrawEnd, withdrawBegin } = useSelector((state: RootState) => state.app)
  ...
  if (!isDuringRestrictTime) {
    return (
      <div>
        <SectionContainer id={"withdraw-section"}>
          ...
          `)}>
              Por favor, preencha o número do CPF corretamente.Se a informação estiver incorreta, o saque
              falhará.Certifique - se de verificar as informações com atenção. Uma conta só pode ser vinculada a um
              número de CPF/CNPJ para saque, uma vez vinculada não pode ser alterada.
            </div>
        ...
        {isMobile && (
          <section
            className={"bg-[rgba(0,0,0,.5)] fixed bottom-0 left-0 right-0 flex flex-col justify-center items-center w-full bg-[rgba(1,62,66,0.6)] py-4 z-10"}>
            {/*<ButtonPro*/}
            {/*  size={"small"}*/}
            {/*  onClick={props.onClickToWithdraw}*/}
            {/*>*/}
    ...
    return (
      <div
        className={tcx('grow h-full w-full flex flex-col justify-center items-center text-center text-lg font-medium text-[var(--secondary-assistant)] py-[64px] px-[240px]', ['text-sm px-3 py-8', isMobile])}>
        <div>Prezado cliente: Olá! Em resposta às exigências do Banco Central do Brasil e do recém-criado comitê de
          agências reguladoras relevantes no Brasil, a plataforma precisa concluir a troca de dados entre o Banco
          Central e as agências reguladoras relevantes das <span className='text-[var(--white)]'>{withdrawBegin}</span>h às <span className='text-[var(--white)]'>{withdrawEnd}</span>h, horário brasileiro!
        </div>
        <br />
        <br />
        <div>Todos os nossos esforços são para garantir que a operação da plataforma esteja mais em conformidade com as
          leis e regulamentos brasileiros relevantes! Proteger a privacidade dos utilizadores e os direitos e interesses
          conexos. Obrigado pela sua compreensão.
        </div>
        <br />
        <br />
        <div>As retiradas serão normais durante outros períodos de tempo na plataforma.</div>
      </div>
    )
  }
}
