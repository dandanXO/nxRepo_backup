...
export const PureLoanDetailsPage = (props: PureLoanDetailsPageProps) => {
    ...
    return (
        <Page>
            {showExtendModal && <ExtendModal setShowExtendModal={setShowExtendModal} {...props.currentData} handlePostRepayCreate={props.handlePostRepayCreate} />}
            ...
            {showRepaymentModal && <RepaymentModal balance={props.currentData.balance} setRepayBalance={setRepayBalance} setShowRepaymentModal={setShowRepaymentModal} setShowRepaymentNoticeModal={setShowRepaymentNoticeModal} handlePostRepayCreate={props.handlePostRepayCreate}/>}
            {showRepaymentNoticeModal && <RepaymentNoticeModal balance={repayBalance} setShowRepaymentNoticeModal={setShowRepaymentNoticeModal} handlePostRepayCreate={props.handlePostRepayCreate}/>}
...
const LoanDetailsPage = () => {
    ...
    const { currentData: repayTypes, isLoading: isRepayTypesLoading, isFetching: isRepayTypesFetching } = useGetRepayTypesQuery({});
    ...
    const postRepayCreateRequest = useCallback((props: PostRepayCreateRequestBody) => {
        postRepayCreate(props).unwrap().then((data: PostRepayCreateResponse) => {
        }).catch(({ error }) => { console.log(error)
        }).finally(() => {});
    }, []);

    const handlePostRepayCreate = useCallback((isExtend: boolean, isForceApplyAfterRepay: true, repayAmount: number) => {
