            ...
            <div>
                <Form onSubmit={this.submit}>
                    <Row gutter={40}>
                        ...
                        <Col lg={12} xl={8}>
                            <Form.Item  {...formItemLayout} label={intl.formatMessage({id :"page.search.list.department.name"})}>
                                {
                                    getFieldDecorator('queryId', {})(
                                        <Select initialValue=''>
                                            <Select.Option key='' value=''><FormattedMessage id="page.search.list.no.restrict" /></Select.Option>
                                            {queryList.length > 0 && queryList.map((item, i) => {
                                                return <Select.Option key={i} value={item.departmentId}>{item.departmentName}</Select.Option>
                                            })}
                                        </Select>
                                    )
                                }
