        ...
        modal.info({
            ...
            content: (
                <WaterMark
                    width={250}
                    zIndex={100}
                    content={[collector, moment().format('YYYY-MM-DD-HH:mm:ss')]}
                    fontSize={32}
                    fontColor={warterMarkColor}
            ...
            <div style={{ margin: '16px' }}>
                ...
                <CardStyle title="认证状态">
                    <Descriptions size="small" bordered layout="vertical" column={{ sm: 4, md: 8 }}>
                        ...
                        <Item label="认证完成时间">
                            {kycFinishTime ? moment(kycFinishTime).format('YYYY-MM-DD HH:mm:ss') : '-'}
                        </Item>
                    </Descriptions>
                </CardStyle>
                <CardStyle title="个人信息">
                    <Descriptions size="small" bordered>
                        ...
                        <Item span={2} label="薪资范围">
                            {salaryRange || '-'}
                        </Item>
                        <Item label="家庭地址">{address || '-'}</Item>
                    </Descriptions>
                </CardStyle>
                {type === 'order' && userRiskControlInfos && (
                    <CardStyle title="风控信息">
                        <Descriptions size="small" bordered>
                            {userRiskControlInfos.map((i, index) => {
                                return (
                                    <>
                                        ...
                                        <Item label="风控分数">
                                            {i?.riskScore === 0 || i?.riskScore ? i?.riskScore : '-'}
                                        </Item>
                                        <Item label="可借款订单数">
                                            {i?.maxLoanApplyCount === 0 || i?.maxLoanApplyCount
                                                ? i?.maxLoanApplyCount
                                                : '-'}
                                        </Item>
                                        <Item label="可借建议金额">
                                            {i?.maxAmount === 0 || i?.maxAmount ? i?.maxAmount : '-'}
                                        </Item>
                                        <Item label="订单风控更新时间">
                                            {i?.orderReviewUpdateTime
                                                ? moment(i?.orderReviewUpdateTime).format('YYYY-MM-DD HH:mm:ss')
                                                : '-'}
                                        </Item>
                                    </>
                                );
