...
export const HelperFormItem = ({
    ...
    rules,
    help,
}: IHelperFormItemProps): JSX.Element => {
    return (
        <Item
            {...layout}
            label={label}
            ...
            help={false}
        >
            <Item name={name} label={label} rules={rules} help="" noStyle>
                {children}
            </Item>
            <Item dependencies={[name]} noStyle>
                {({ getFieldError }) => {
                    const filedErrors = getFieldError(name);
                    if (filedErrors.length > 0) {
                        return (
                            <div style={{ color: 'red', lineHeight: 1.5715 }}>
                                {filedErrors.map((part) => (
