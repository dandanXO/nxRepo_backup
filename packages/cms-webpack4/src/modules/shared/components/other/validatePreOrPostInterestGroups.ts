...
function equalRangeBelow100(str: string, min = 0, max = 100): boolean {
    return Number(str) < min || Number(str) > max;
}
export const validateValue = (value: string | number, errorText: string): string => {
    return value !== 0 && !value
        ? errorText
        : isNaN(value as number)
        ? '请输入數字'
        : equalRangeBelow100(value as string)
        ? '请输入0-100间数字'
        : '';
};

export const validateNum = (value: string | number, errorText: string): string => {
    return value !== 0 && !value
        ? errorText
        : isNaN(value as number)
        ? '请输入數字'
...
export const validateplusAmount = (value: string | number, errorText: string): string => {
    return value !== 0 && !value
        ? errorText
        : isNaN(value as number)
        ? '请输入數字'
