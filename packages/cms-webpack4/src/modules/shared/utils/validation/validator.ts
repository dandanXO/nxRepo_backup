...
export const NumberValidator =
    ...
    (params: ValidateNumber): Promise<string> => {
        // console.log("value", value);
        // console.log("params", params);
        if (params.required) {
            const stringScheme = z
                .string()
                .min(
                    1,
                    params && params.requiredErrorMessage
                        ? params.requiredErrorMessage
                        : RequireNumberMessage(params.min),
                );
...
export const EmailValidator =
    (_: RuleObject, value: StoreValue) =>
    (params: ValidateEmail): Promise<string | void> | Promise<void> => {
        const scheme = z.string({}).email({ message: params.typeErrorMessage || '请输入正确的邮箱格式' });
        if (params.required) {
...
export const TagValidator =
    (_: RuleObject, value: StoreValue) =>
    (params: ValidateTag): Promise<string | void> | Promise<void> => {
        // console.log("TagValidator.value", value);
        const scheme = z.string().array().min(1, params.typeErrorMessage).max(3, params.typeErrorMessage);
