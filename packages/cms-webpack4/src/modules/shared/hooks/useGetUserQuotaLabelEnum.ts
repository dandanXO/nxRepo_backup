...
const useGetUserQuotaLabelEnum = (): {
    ...
    // 注册渠道
    const [triggerGetUserQuotaLable, { currentData: userQuotaLableData, isFetching }] =
    ...
    useEffect(() => {
        if (userQuotaLableData) {
            ...
            userQuotaLableData &&
                userQuotaLableData?.map((i) => {
                    return userQuotaLablEnumData.set(i.id, {
                        text: i.labelName,
                        color: colorEnum[i.labelColor].color,
                        id: i.id,
                    });
                });
            ...
            setUserQuotaLablSelect([
                { value: '', label: '额度标签' },
                ...userQuotaLablSelectData,
                { value: 0, label: '移除配置' },
            ]);
        }
    }, [isFetching]);
