...
const customZodNumberRules = (name: string) =>
    z
        .number({
            // eslint-disable-next-line camelcase
            required_error: `${REQUIRED_MESSAGE}${name}`,
...
export class ChannelTagSchemaEntity extends SchemaEntity<IChannelTagSchema> {
    ...
    transformToEntityData(sourceData: IChannelTagSchema): IChannelTagSchema {
        return {
            ...
            auditLoanAmount: !isNaN(sourceData.auditLoanAmount)
                ? Number(sourceData.auditLoanAmount)
                : sourceData.auditLoanAmount,
            // 审核的借款金额

            auditQuota: !isNaN(sourceData.auditQuota) ? Number(sourceData.auditQuota) : sourceData.auditQuota,
            // 审核的订单额度

            auditServiceFee: !isNaN(sourceData.auditServiceFee)
                ? Number(sourceData.auditServiceFee)
                : sourceData.auditServiceFee,
            // 审核的服务费
