...
export const TreeCheckbox =  (props: TreeCheckboxProps): JSX.Element => {
    ...
    // console.log("checkChange.e", e)
        ...
        // NOTE: 當前選擇催收階段
        const currentSelectedStage = data && data.find((d) => d.key === parentRowsKey[0]);
        // console.log("currentSelectedStage", currentSelectedStage);

        // NOTE: 全部商戶
        const currentAllMerchant = currentSelectedStage && currentSelectedStage.children.map((d) => d.key);
        ...
        // NOTE: 全部 Team
        const currentAllTeam = currentSelectedAllMerchant && currentSelectedAllMerchant.children.map((d) => d.key);
        ...
        if (e.target.checked) {
            ...
            //NOTE: 動作選中人員
            const newCheckedJob = [...props.checkedJob, e.target.value];
