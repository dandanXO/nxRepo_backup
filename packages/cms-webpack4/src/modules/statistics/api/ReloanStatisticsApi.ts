...
import {
    ...
    GetReloanStatisticsListResponse,
} from './types/ReloanStatisticsTypes/getReloanStatisticsList';

const ReloanStatisticsApi = API.injectEndpoints({
    overrideExisting: false,
    endpoints: (builder) => ({
        // NOTE: GET /hs/admin/statistics/reloan-statistics 用户复借统计
        getReloanStatisticsList: builder.query<
            GetReloanStatisticsListResponse,
            GetReloanStatisticsListRequestQuerystring
        >({
...
export const { useLazyGetReloanStatisticsListQuery } = ReloanStatisticsApi;
