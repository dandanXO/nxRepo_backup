...
const OrderFinalReviewTable = (): JSX.Element => {
    ...
    // api
    const [triggerGetList, { currentData, isFetching }] = useLazyGetOrderFinalReviewListQuery({
        pollingInterval: 0,
        refetchOnFocus: false,
        refetchOnReconnect: false,
    });
    const [postOrderFinalReview, { isSuccess: postOrderReviewIsSuccess }] = usePostOrderFinalReviewMutation();
    ...
    const handleToUserDetail = (userId, orderId, orderNo) => {
        history.push(`/orderManagement/businessLastCheck/review/${userId}/${orderId}/${orderNo}`);
        savePath('/orderManagement/businessLastCheck', '/orderManagement/businessLastCheck/review');
    };
            ...
            headerTitle={
                <Space>
                    ...
                    {appInfo.COUNTRY === 'Bangladesh' && (
                        <>
                            <Button
                                key="reject7daysButton"
                                type="primary"
                                ghost
                                disabled={buttonDisabled}
                                onClick={() => handleReviewAll(3)}
                            >
                                全部拒绝
                            </Button>
                            <Button
                        ...
                        <Button
                            ...
                            onClick={() => {
                                ...
                                setSearchList({
                                    ...
                                    appName,
                                    applyChannel:
                                        applyChannel === '' ? '' : channelListEnum?.get(applyChannel)?.text || '',
                                    phoneNo,
                                    oldMember,
                                    orderNo,
                                    productName,
